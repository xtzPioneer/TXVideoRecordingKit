 ...
 */
#if DEBUG
#define TXVIRELog(s, ... ) NSLog( @"<FileName:%@ InThe:%d Line> Log:%@", [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __LINE__, [NSString stringWithFormat:(s), ##__VA_ARGS__] )
#else
#define TXVIRELog(s, ... )
#endif
...
@interface TXCameraController ()<AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate, UIGestureRecognizerDelegate>
...
@property (strong, nonatomic) TXCameraButton *cameraButton;//拍摄按钮
...
@property (assign, nonatomic) Boolean isRotatingCamera;//正在旋转摄像头,捏合缩放摄像头
...
@implementation TXCameraController{
...
+ (instancetype)defaultCameraController{
    TXCameraController *cameraController=[[TXCameraController alloc] initWithNibName:@"TXCameraController" bundle:nil];
    return cameraController;
}

- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil{
    NSBundle *bundle=[NSBundle bundleForClass:self.class];
    NSString *path=[bundle pathForResource:@"TXVideoRecordingKit" ofType:@"bundle"];
    NSBundle *xibBundle=path?[NSBundle bundleWithPath:path]:[NSBundle mainBundle];
    if (self = [super initWithNibName:@"TXCameraController"
                               bundle:xibBundle]) {
    }
    return self;
}
...
- (IBAction)confirmBtnFunc:(id)sender{
    ...
    if (self.photoPreviewImageView){
        ...
        [TXPhotoLibraryManager savePhotoWithImage:finalImage andAssetCollectionName:self.assetCollectionName withCompletion:^(UIImage *image, NSError *error) {
        ...
        [weakSelf cropWithVideoUrlStr:weakSelf.videoURL start:0 end:weakSelf.currentVideoTimeLength completion:^(NSURL *outputURL, Float64 videoDuration, BOOL isSuccess) {
            if (isSuccess){
                [TXPhotoLibraryManager saveVideoWithVideoUrl:outputURL andAssetCollectionName:nil withCompletion:^(NSURL *videoUrl, NSError *error) {
...
- (void)setupAudio{
    ...
    if (error){
        TXVIRELog(@"取得设备输入audioInput对象时出错，错误原因：%@", error);
        return;
    }
...
- (void)configDefaultUIDisplay{
    ...
    if (_cameraButton == nil){
        TXCameraButton *cameraButton = [TXCameraButton defaultCameraButton];
...
- (void)changeDeviceProperty:(PropertyChangeBlock)propertyChange{
    ...
    if ([captureDevice lockForConfiguration:&error]){
        ...
        TXVIRELog(@"设置设备属性过程发生错误，错误信息：%@",error.localizedDescription);
    }
}
...
- (void)previewPhotoWithImage:(UIImage *)image{
    ...
    //    TXVIRELog(@"image %@",NSStringFromCGSize(image.size));
    //    TXVIRELog(@"finalImage %@",NSStringFromCGSize(finalImage.size));
...
- (void)setUpWriter{
    ...
    if ([_assetWriter canAddInput:_assetWriterVideoInput]){
        ...
        TXVIRELog(@"AssetWriter videoInput append Failed");
    }
    
    if ([_assetWriter canAddInput:_assetWriterAudioInput]){
        [_assetWriter addInput:_assetWriterAudioInput];
    }else{
        TXVIRELog(@"AssetWriter audioInput Append Failed");
    }
...
- (void)timerRecord{
    ...
    //    TXVIRELog(@"%lf", timeLength / VIDEO_RECORDER_MAX_TIME);
...
- (UIImage *)thumbnailImageRequestWithVideoUrl:(NSURL *)videoUrl andTime:(CGFloat)timeBySecond{
    ...
    if(error){
        TXVIRELog(@"截取视频缩略图时发生错误，错误信息：%@", error.localizedDescription);
        return nil;
    }
        ...
        [exportSession exportAsynchronouslyWithCompletionHandler:^{
            switch ([exportSession status]) {
                case AVAssetExportSessionStatusFailed:{
                    TXVIRELog(@"合成失败：%@", [[exportSession error] description]);
...
- (void)requestAuthorizationForPhotoLibrary{
    ...
    // 请求照片权限
    [TXPhotoLibraryManager requestALAssetsLibraryAuthorizationWithCompletion:^(Boolean isAuth) {
